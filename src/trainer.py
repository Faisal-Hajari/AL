...
class LOTClassTrainer(object): 
    ...
    def __init__(self, args): 
        ...
        eff_batch_size = self.train_batch_size * self.accum_steps
        ...
        self.model = LOTClassModel(num_labels= self.num_class)
        ...
        self.with_test_labels=True

    
    #prints truncation stats of the document
    ...
    #list[str] -> (encoded_str[tensor], attention_masks[tensor])
        ...
        if find_label_name: 
            ...
            if os.path.exists(loader_file): 
                ...
                label_name_data = torch.load(loader_file)
                    ...
                    input_ids_with_label_name, attention_masks_with_label_name, label_name_idx = self.label_name_occurrence(docs=docs)
    ...
    def label_name_in_doc(self, doc): 
        ...
        new_doc = []
        wordpcs = []
        idx =1 # index starts at 1 due to [CLS] toke
...
#def create_dataset(self, dataset_dir, text_file, label_file, loader_name, find_label_name=False, label_name_loader_name=None):
    ...
    def make_dataloader(self, rank:int = 0, data_dict=None, batch_size=None): 
    ...
    def filter_keywords(self, category_vocab_size:int=100):
        ...
        for i, cat_dict in self.category_words_freq.items(): 
            ...
            stopwords_vocab = arabic_stopwords.words
            for i, word_list in self.category_vocab.items(): 
                ...
                for j, word_id in enumerate(word_list):
                    ...
                    if word in self.label_name_dict[i]:
                        continue
                    #if not self.isalpha(word) or len(word) == 1 or word in stopwords_vocab or word_id in repeat_words: #TODO: implement self.isAlpha() -> takes str return ture if it's all abjd false otherwise
                    if len(word) == 1 or word in stopwords_vocab or word_id in repeat_words:    
                        delete_idx.append(j)
                self.category_vocab[i] = np.delete(self.category_vocab[i], delete_idx)
    

    # construct category vocabulary (distributed function)
    def category_vocabulary_dist(self, rank:int = 0, top_pred_num=50, loader_name="category_vocab.pt"):
        model = self.model
        model.eval()
        label_name_dataset_loader = self.make_dataloader(rank, self.label_name_data, self.eval_batch_size)
        category_word_freq = {i: defaultdict(float) for i in range(self.num_class)}
        ...
        try: 
            for batch in wrap_label_name_dataset_loader: 
                with torch.no_grad(): 
                    ...
                    #print("$$")
                    _, sorted_res = torch.topk(predictions[0][match_idx], top_pred_num, dim=-1)
                    label_idx = label_pos[match_idx]
                    for i, word_list in enumerate(sorted_res): 
                        for j, word_id in enumerate(word_list): 
                            category_word_freq[label_idx[i].item()][word_id.item()] += 1
            save_file = os.path.join(self.temp_dir, f"{rank}_"+loader_name)
            torch.save(category_word_freq, save_file)
    ...
    def category_vocabulary(self, top_pred_num=50, category_vocab_size=100, loader_name="category_vocab.pt"):
        ...
        if os.path.exists(loader_file): 
            ...
            self.category_vocabulary_dist(top_pred_num=top_pred_num, loader_name=loader_name)
            result = []
            for f in os.listdir(self.temp_dir): 
                if f[-3:]=='.pt': 
                    result.append(torch.load(os.path.join(self.temp_dir, f)))

                assert len(result) == 1, "ERROR Gathering results, check category_vocabulary"
                self.category_words_freq = {i:defaultdict(float) for i in range(self.num_class)}
                for i in range(self.num_class): 
                    for category_words_freq in result:
    ...
    def prepare_mcp_dist(self, rank:int=0, top_pred_num=50, match_threshold=20, loader_name="mcp_train.pt"):
        model = self.model
        model.eval()
        ...
        try: 
            for batch in wrap_train_dataset_loader: 
                with torch.no_grad(): 
                    ...
                    _, sorted_res = torch.topk(predictions[0], top_pred_num, dim=-1)
                    for i, category_vocab in self.category_vocab.items():
            ...
            print('oops!, error in prepare_mcp_dist')
    ...
    def prepare_mcp(self, top_pred_num=50, match_threshold=20, loader_name="mcp_train.pt"):
        ...
        if os.path.exists(loader_file): 
            ...
            if not os.path.exists(self.temp_dir): 
                os.makedirs(self.temp_dir)
            self.prepare_mcp_dist(top_pred_num=top_pred_num, match_threshold=match_threshold, loader_name=loader_name)
            result = [] 
            for f in os.listdir(self.temp_dir): 
                if f[-3:] == '.pt': 
                    result.append(torch.load(os.path.join(self.temp_dir, f)))

            assert len(result) == 1, 'ERROR at prepare_mcp'
            all_input_ids = result[0]["all_input_ids"]
            all_mask_label = result[0]["all_mask_label"]
            all_input_mask = result[0]["all_input_mask"]
    ...
    def mcp_dist(self, rank:int=0, epochs:int=5, loader_name="mcp_model.pt"):
        model = self.model
        ...
        try: 
            for i in range(epochs): 
                ...
                total_train_loss = 0
                print(f"Epoch {i+1}:")
                wrap_mcp_dataset_loader = tqdm(mcp_dataset_loader)
                model.zero_grad()
                ...
                avg_train_loss = torch.tensor([total_train_loss/len(mcp_dataset_loader) *self.accum_steps])
                print(f"Average training loss: {avg_train_loss.mean().item()}")
            
            loader_file = os.path.join(self.dataset_dir, loader_name)
            torch.save(model.state_dict(), loader_file)
        except RuntimeError as err: 
            print('oops!, error at mcp_dist')



    # masked category prediction 
    def mcp(self, top_pred_num=50, match_threashold=20, epochs=5, loader_name="mcp_model.pt"):
        loader_file = os.path.join(self.dataset_dir, loader_name)
        if os.path.exists(loader_file):
            print(f"\nLoading model trained via masked category prediction from {loader_file}")
        else: 
            self.prepare_mcp(top_pred_num, match_threashold)
            print(f"\nTraining model via masked category prediction.")
            self.mcp_dist(epochs=epochs, loader_name=loader_name)
        
        self.model.load_state_dict(torch.load(loader_file))


    # prepare self training data and target distribution
    def prepare_self_train_data(self, rank:int=0, model=None, idx=None):
        target_num = min(self.train_batch_size * self.update_interval * self.accum_steps, len(self.train_data["input_ids"]))
        ...
        input_ids, input_mask, all_preds = self.inference(model, dataset_loader, rank, return_type="data")
    ...
    def self_train_batches(self, rank, model, self_train_loader, optimizer, scheduler, test_dataset_loader): 
        ...
        try: 
            ...
            print("error at  self_train_batches")


    def self_train_dist(self, rank:int=0, epochs=0, loader_name="final_model.pt"):
        model = self.model
        test_dataset_loader = self.make_dataloader(rank, self.test_data, self.eval_batch_size) if self.with_test_labels else None
        total_steps = int(len(self.train_data["input_ids"])*epochs / (self.train_batch_size * self.accum_steps))
    ...
    def self_train(self, epochs, loader_name="final_model.pt"): 
        loader_file = os.path.join(self.dataset_dir, loader_name)
        if os.path.exists(loader_file): 
            print(f"\nFinal model {loader_file} found, skipping self-training")
        else: 
            ...
            print(f"\nStart self-training")
            self.self_train_dist(epochs=epochs, loader_name=loader_name)
    

    def inference(self, model, dataset_loader, rank:int=0, return_type=None): 
        if return_type == "data": 
            all_input_ids = []
            all_input_mask =[]
            all_preds = []
